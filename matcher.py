...
# Try to load ANN model with multiple format support
try:
    from tensorflow.keras.models import load_model
    
    # Try different model formats
    if os.path.exists("resume_score_ann.keras"):
        ann_model = load_model("resume_score_ann.keras")
        print("✅ ANN model loaded successfully (.keras format)")
    elif os.path.exists("resume_score_ann"):
        ann_model = load_model("resume_score_ann")
        print("✅ ANN model loaded successfully (folder format)")
    else:
    ...
    ann_model = None
    scaler = None
    print(f"⚠️ ANN model not found, using rule-based scoring: {e}")
...
def compute_similarity(resume_text: str, jd_text: str) -> float:
    """
    Compute match score using ANN if available, else rule-based
    """
    if not resume_text or not jd_text:
        return 0.0
    ...
    # Semantic similarity
    try:
        embeddings = model.encode([resume_text[:1000], jd_text[:1000]], 
                                  convert_to_tensor=True)
        semantic_similarity = util.cos_sim(embeddings[0], embeddings[1]).item()
    except Exception:
        semantic_similarity = skill_match_ratio

    # Use ANN if available
    if ann_model is not None and scaler is not None:
        try:
            features = np.array([[skill_match_ratio, semantic_similarity,
                                 num_resume_skills, num_jd_skills]])
            features_scaled = scaler.transform(features)
            score = float(ann_model.predict(features_scaled, verbose=0)[0][0])
            score = np.clip(score, 0, 100)
            
            # Post-processing: Cap score based on skill match
            if skill_match_ratio < 1.0:
                max_possible_score = 85 + (skill_match_ratio * 15)
                score = min(score, max_possible_score)
            
            print(f"✅ ANN PREDICTION USED: Input={features[0]}, Raw Score={score:.2f}")
            return round(score, 2)
        except Exception as e:
            print(f"⚠️ ANN prediction failed: {e}, using fallback")
    
    # Fallback: Enhanced rule-based scoring
    print("⚠️ FALLBACK METHOD USED (No ANN)")
    base_score = skill_match_ratio * 70
    semantic_component = semantic_similarity * 20
    skill_bonus = min(10, (num_resume_skills / 30) * 10)
    
    final_score = base_score + semantic_component + skill_bonus
...
# Get Feature Details
# --------------------------------------------
def get_feature_details(resume_text: str, jd_text: str):
    ...
    skill_match_ratio = len(overlapping_skills) / len(jd_skills) if jd_skills else 0

    return {
        "resume_skills": list(resume_skills),
        "jd_skills": list(jd_skills),
        "common_skills": list(overlapping_skills),
